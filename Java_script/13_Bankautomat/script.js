...
while (!isfinished) {
  ...
  if (Selection == 1) {
    ...
    if (Einzahlen > 0) {
      ...
      console.log(
        "Ihre Einzahlung war Erfolgreich. Neuer Kontostand: " + kontostand + "€"
      );
    } else if (Einzahlen != Number || Einzahlen <= 0) {
  ...
  if (Selection == 2) {
    ...
    if (isNaN(Abheben) || Abheben <= 0) {
      console.log("Ungültige Eingabe. Bitte geben Sie eine Zahl ein.");
    } else {
      if (Abheben > kontostand) {
        console.log(
          "Der Betrag konnte nicht abgehoben werden. Nicht genügend Guthaben vorhanden."
        );
      } else {
        kontostand -= Abheben;
        console.log(
          "Erfolgreich abgehoben. Neuer Kontostand: " + kontostand + "€"
        );
      }
    }
  }

  if (Selection == 3) console.log("Ihr Kontostand Beträgt:" + kontostand + "€");
